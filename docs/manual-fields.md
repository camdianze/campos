...
제목 옆에 **지금 보이는 상품 수**가 적힌다(`124 products`). 검색·필터를 걸면 그 결과 개수로 함께 줄어든다.
...
> **화면에 들어오면 커서가 이 칸에 자동으로 놓인다.** 아무것도 누르지 않고 바로 찍으면 된다.
>
> 그리고 **커서가 다른 데 있어도 스캔은 이 칸으로 들어온다.** 버튼이나 배치 드롭다운을 한 번 누르면 커서가 검색창을 떠나는데, 그 상태의 스캔이 아무 데도 안 들어가고 사라지면 계산대에서는 "찍었는데 아무 일도 안 일어난다"로만 보이고 원인을 알 수가 없다. 그래서 글자가 들어오면 검색창으로 돌려보낸다.
>
> 예외 둘 — **수량·단가·받은 돈 칸에 직접 타이핑 중일 때**와 **배치 드롭다운을 펼쳐 놓고 고르는 중일 때**는 가로채지 않는다. 스페이스도 그대로 둔다(버튼을 누르는 키라서).
...
| 1 | `Product` | 상품명 |
| 2 | `Batch` | 배치번호 |
| 3 | `Qty` | 수량. **칸에 바로 입력해서 고칠 수 있다** — Enter 또는 다른 곳 클릭이 적용, Esc는 취소, `0`은 그 줄 삭제. 재고 검사는 담을 때와 같은 규칙(재고 초과·안 뜯은 박스 부족이면 원래 수량으로 되돌아가고 하단 메시지, 박스를 헐어야 하면 `Open a Box` 확인창). 적용 뒤 커서는 검색창으로 돌아간다 |
...
### 24-5. 왼쪽 아래 패널 — 12개월 그래프 / 매출 달력

같은 자리를 두 그림이 나눠 쓴다. 패널 오른쪽 위의 `12 months` / `Calendar` 버튼으로 바꾸며, 켜져 있는 쪽이 파랗다. 기본은 `12 months`.

**`Calendar`** — 한 달을 날짜별로 펼친 달력이다.

| 컨트롤 | 하는 일 |
|---|---|
| `◀` / `▶` | 달을 넘긴다. **고른 기간과는 별개로 움직이고**, 넘길 때 표와 요약 카드는 다시 조회하지 않는다. `▶`는 이번 달에서 비활성 — 오지 않은 달은 없다 |
| 달 이름 옆 문구 | 그 달의 합계 `{금액} · {n} transactions`. 판매가 없으면 `no sales this month` |
| 날짜 칸 | 오른쪽 아래에 **반올림한 그 날 순매출**. 판매가 없는 날은 빈칸. 마우스를 올리면 정확한 금액·건수가 뜬다 |

- 달력을 처음 켜면 **고른 기간이 끝나는 달**이 나온다. 지난달 리포트를 보다 켰는데 이번 달이 나오면 방금 보던 것과 어긋나기 때문이다.
- **날짜를 누르면 팝업이 아니라 기간 자체가 그 하루로 바뀐다.** 요약 카드·상품 순위·항생제 표가 전부 그 날 기준이 된다. 줄 단위 내역(영수증 재출력·환불)은 판매 이력 화면이 맡으므로 여기서 되풀이하지 않는다.
- 달력을 켜 둔 채 기간을 바꾸면 달력도 그 기간이 끝나는 달로 따라간다.
- 금액 규칙은 12개월 그래프와 **글자 그대로 같다** — 환불 차감한 순매출, 건수는 판매만, 날짜 경계는 로컬 자정. 그래서 **칸을 다 더한 값이 그 달을 고른 `Sales Amount` 카드와 일치한다** (`DailySalesTests`가 이것을 고정한다).
...
**낱개가는 소수점 네 자리까지 된다.** 500리엘짜리 알약은 $0.125라 센트로 적을 수 없다 — 두 자리로 막으면 그런 상품이 등록되지 않는다.

**손으로 채운 표기를 읽어 준다.** `units_per_box`에 `0`이나 `1`은 "낱개로 안 판다"는 뜻이라 빈칸과 같게 읽고, 숫자 칸의 `-` · `–` · `n/a`는 "없음"으로 읽는다. 필수 칸이 그렇게 비면 `… is empty. It is required for a new product.`로 잡히므로 무엇을 채워야 하는지 알 수 있다.
...
**바코드가 없으면 `product_name` + `manufacturer`로 가른다.** 이름이 같아도 제조사가 다르면 **다른 상품**으로 등록된다. 제조사 칸이 **빈 행은 이름만으로** 기존 상품을 찾는다 — 배치가 여럿인 상품의 2번째 행이 그렇게 생겼기 때문이다. 제조사 없이 먼저 등록해 둔 상품이 하나뿐이면, 제조사를 적은 행은 **그 상품에 제조사를 채운다**(새로 만들지 않는다). 이름이 같은 상품이 둘 이상인데 행에 제조사가 없으면 그 행은 거부된다: `'…' matches 2 products. Fill in manufacturer to say which one this row is.`

**2단계(재고)도 같은 규칙이다.** 이름이 같은 상품이 둘이면 `manufacturer`를 보고 그쪽 선반에 넣는다. 제조사를 안 적으면 그 행은 거부되고, 등록되지 않은 제조사면 `'…' from Maker B is not registered. Import products first.`로 잡힌다.
