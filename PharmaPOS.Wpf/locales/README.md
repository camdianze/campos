...
`ui.`로 시작하는 키는 **화면 글자**다. 복약안내·영수증과 달리 **`review_status`로 막지 않는다.**

인쇄물과 위험이 다르기 때문이다. 복약 문구는 환자가 그대로 따라 하는 글이라 틀리면 되돌릴 방법이 없지만, 버튼에 적힌 낱말이 어색한 것은 계산대에서 바로 눈에 띄고 고치면 그만이다. 검수를 기다리는 동안 기능이 아예 동작하지 않으면 검수할 거리도 확인할 수 없다.

**화면에는 검수 여부를 알리는 표시가 없다.** 한때 미검수 번역을 붉은 글씨로 칠했지만, 계산대에서 매일 보는 화면이 온통 붉게 나오는 쪽이 알리려던 것보다 더 방해가 되어 걷어냈다. 검수 상태는 이 파일의 `review_status`가 말해 준다.

번역이 없는 키는 **영어가 그대로 나온다.** 빈 버튼이나 키 이름이 화면에 뜨는 쪽이 훨씬 나쁘다. 그래서 새 화면을 번역할 때 키를 다 채우지 않아도 된다.

| 키 | 자리 |
|---|---|
| `ui.products` / `ui.inventory` / `ui.pos_sale` | 메인 화면 카드 세 개 |
| `ui.pos.*` | POS 화면 주요 버튼 (담기·확정·취소·삭제·뒤로) |
